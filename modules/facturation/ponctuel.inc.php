<?
// ---------------------------------------------------------------------------------------------
//   Saisie d'une pr�sence ponctuelle
//     ($Author: miniroot $)
...
<?
	if (!GetDroit("AccesPresence")) { FatalError("Acc�s non autoris� (AccesPresence)"); }
...
// ---- R�cup�re la liste des mouvements
...
// ---- Enregistre
	...
	if ($fonc=="Enregistrer")
	{
		...
		// D�fini le type de jour pour la pr�sence
		...
		if (is_array($form_poste))
		{
				...
				foreach($form_poste as $i=>$ligne)
				  { 
					if ($ligne>0)
					{
						...
						if ($res["j".$todayNum]=="")
						  {
						  	$msg_result.="Pas de type de pr�sence d�fini pour le ".$tabJour[$todayNum]."<br \>";
						  }
							...
							if ($f=="")
							  {
						  		...
						  		$query.="commentaire='Pr�sence ponctuelle le ".$form_dte."', ";
			...
			$msg_result.="<font color=\"green\"><b>La pr�sence ponctuelle a �t� enregistr�e.</b></font><br />";
		}
