...
<?
	if (!GetDroit("AccesAbonnements")) { FatalError("Acc�s non authoris� (AccesAbonnements)"); }
...
// ---- Mets � jour les lignes en cas de refresh
// && ($fonc=="Enregistrer"))
	...
	if (($fonc=="facturer") && ($id>0))
	  {
		...
		if (count($tabValeur)>0)
		  {		
			...
			$tmpl_x->assign("msg_confirmation", "L'imputation sur le compte a �t� faites.");
